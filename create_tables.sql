...
-- Cambio de Adry: Catálogo de estados para las citas (Programada, Completada, Cancelada, etc.)
...
-- Cambio de Adry: Catálogo de motivos cuando se cancela una cita
...
-- Cambio de Adry: Catálogo de especialidades médicas (Cardiología, Pediatría, etc.)
...
-- Cambio de Adry: Tabla para gestionar múltiples clínicas del sistema
-- Permite seleccionar la clínica al crear una cita
...
-- Cambio de Adry: Tabla principal de pacientes del sistema
...
-- Cambio de Adry: Tabla para horarios estándar de la clínica
-- Define horarios de Lunes a Viernes
CREATE TABLE AGENDA_HORARIO (
    ...
    -- Cambio de Adry: Columna NOMBRE renombrada a HORARIO para descripción del horario
...
-- Cambio de Adry: Tabla principal de empleados del sistema
-- Agregados campos: PRIMER_APELLIDO, SEGUNDO_APELLIDO, HORARIO_TRABAJO
-- HORARIO_TRABAJO es FK a AGENDA_HORARIO para asignar horario laboral
CREATE TABLE PERSONAL (
    ID_PERSONAL        NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY PRIMARY KEY,
    PRIMER_NOMBRE      VARCHAR2(50) NOT NULL,
    SEGUNDO_NOMBRE     VARCHAR2(50),
    -- Cambio de Adry: Agregados apellidos como campos separados
    PRIMER_APELLIDO     VARCHAR2(50) NOT NULL,
    SEGUNDO_APELLIDO    VARCHAR2(50),
    PUESTO             VARCHAR2(50),
    ...
    -- Cambio de Adry: Agregado campo para horario de trabajo del personal (FK a AGENDA_HORARIO)
...
-- Cambio de Adry: Tabla que relaciona personal que es médico
-- Agregada columna ID_HORARIO para asignar horarios de atención médica
CREATE TABLE MEDICO (
    ID_MEDICO      NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY PRIMARY KEY,
    ID_PERSONAL    NUMBER NOT NULL,
    -- Cambio de Adry: Agregado ID_HORARIO para definir horario de atención del médico
    ID_HORARIO     NUMBER(2),
...
-- Cambio de Adry: Tabla para asignar especialidades a los médicos
...
-- Cambio de Adry: Tabla para gestionar consultorios por clínica
-- Cada consultorio pertenece a una clínica 
...
-- Cambio de Adry: Tabla para asignar médicos a consultorios con horarios específicos
-- Agregada columna ID_HORARIO para relacionar con AGENDA_HORARIO
CREATE TABLE AGENDA_MEDICA (
    ...
    -- Cambio de Adry: Agregada columna ID_HORARIO para relacionar con AGENDA_HORARIO
    ID_HORARIO        NUMBER(2),
    VIGENTE_DESDE     DATE,
    VIGENTE_HASTA     DATE,
...
-- Cambio de Adry: Tabla central para gestión de citas médicas
CREATE TABLE CITA (
    ID_CITA               NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY (START WITH 1 INCREMENT BY 1) PRIMARY KEY,
    ID_PACIENTE           NUMBER NOT NULL,
    ID_MEDICO             NUMBER NOT NULL,
    -- Cambio de Adry: ID_CONSULTORIO es obligatorio para saber en qué consultorio será la cita
    ID_CONSULTORIO        NUMBER NOT NULL,
...
-- Cambio de Adry: Tabla para registrar la atención médica realizada en una cita
...
-- Cambio de Adry: Tabla para gestionar contratos del personal
...
-- Cambio de Adry: Tabla para expedientes médicos de los pacientes
--  fecha de creación y notas del expediente
...
-- CAMBIOS  ADRY
-- =====================================================
-- 1. CLINICA: Nueva tabla para gestionar múltiples clínicas
-- 2. CONSULTORIO: Asociado a CLINICA mediante FK
-- 3. AGENDA_HORARIO: Nueva tabla con horarios estándar (Lunes-Viernes, Mañana/Tarde)
--    - Columna NOMBRE renombrada a HORARIO
-- 4. PERSONAL: 
--    - Agregados campos PRIMER_APELLIDO y SEGUNDO_APELLIDO
--    - HORARIO_TRABAJO cambió de VARCHAR2 a NUMBER(2) con FK a AGENDA_HORARIO
-- 5. MEDICO: 
--    - Agregada columna ID_HORARIO con FK a AGENDA_HORARIO
-- 6. AGENDA_MEDICA: 
--    - Agregada columna ID_HORARIO con FK a AGENDA_HORARIO
-- 7. CITA: 
--    - ID_CONSULTORIO es obligatorio (NOT NULL)
--    - Integración completa con clínicas y consultorios
-- =====================================================

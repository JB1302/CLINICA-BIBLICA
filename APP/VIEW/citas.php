...
<body class="d-flex flex-column min-vh-100">
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select" name="id_estado" required>
                                    ...
                                    <option value="<?= htmlspecialchars($e['ID_ESTADO']) ?>"
                                        <?= (strtolower($e['NOMBRE_ESTADO']) == 'programada') ? 'selected' : '' ?>>
                    ...
                    <form id="formEditarCita" method="post" action="citas.php">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-8">
                                ...
                                <textarea class="form-control" name="id_motivo_cancelacion" rows="2"
                                    placeholder="Solo si aplica"></textarea>
                            </div>
                    ...
                    <form id="formCancelarCita" method="post" action="citas.php">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" name="observaciones" rows="3"
                                placeholder="Opcional - Agregue detalles adicionales si lo desea"></textarea>
                        </div>
                    ...
                    <form id="formEliminarCita" method="post" action="citas.php">
                        ...
                        <p class="mb-3">
                            ...
                            <small class="text-muted">
                                Esta acción elimina la cita definitivamente (solo es posible si no tiene atenciones
                                registradas).
                            </small>
                        </p>
    ...
    <script>
    // Modales: Cancelar / Eliminar
    // inyecta el ID de la cita en el formulario
    // Al cargar el DOM, registramos listeners de Bootstrap para precargar data en modales
    document.addEventListener('DOMContentLoaded', () => {
        // Modal CANCELAR
        // cuando se va a mostrar el modal, llenamos los datos
    ...
    // Llenar datos al abrir y limpiar al cerrar
    ...
    if (modalEditar) {
        ...
        modalEditar.addEventListener('show.bs.modal', function(event) {
            ...
            //  Datos capturados (vienen del row de la tabla)
    ...
    function cargarHorariosMedico(idMedico, tipo) {
        // Si no hay médico seleccionado, ocultar info y salir
        if (!idMedico) {
        ...
        fetch(url)
            ...
            .then(data => {
                ...
                if (data.success && data.horarios && data.horarios.length > 0) {
                    ...
                    horariosHtml +=
                        '<br><small class="text-warning"><i class="fa-solid fa-triangle-exclamation me-1"></i>Asegúrate de que las horas de la cita estén dentro de este horario.</small>';
                    ...
                    textoDiv.innerHTML =
                        '<em class=\"text-muted\">Este médico no tiene horarios configurados.</em>';
                    infoDiv.classList.remove('d-none');
                    if (tipo === 'nuevo') horariosNuevo = null;
                    if (tipo === 'editar') horariosEditar = null;
                }
            ...
            .catch(error => {
                ...
                const infoDiv = document.getElementById('horarios-info-' + tipo);
                const textoDiv = document.getElementById('horarios-texto-' + tipo);
                textoDiv.innerHTML = '<em class=\"text-danger\">Error al cargar horarios: ' + error.message +
                    '</em>';
                infoDiv.classList.remove('d-none');
                if (tipo === 'nuevo') horariosNuevo = null;
                if (tipo === 'editar') horariosEditar = null;
    ...
    // mantiene el modal y muestra errores sin recargar
    ...
    // Datos de consultorios por clínica (desde PHP)
    ...
    <script>
    // Limpiar parámetros de URL después de mostrar alerta global
    document.addEventListener("DOMContentLoaded", () => {
    ...
    <script>
    // Eliminar alerta global después de 3 segundos
    setTimeout(() => {
        const alertGlobal = document.getElementById('alertGlobal');
